...
const Carousel = React.forwardRef<CarouselRef, CarouselProps>((props, ref) => {
    ...
    const itemSize = React.useRef<number>();
    const itemSizes = React.useRef<Map<number, number>>();
    ...
    const handleTouchStart = (event: TouchEvent) => {
        if (!enableNavigationGestures && direction === 'horizontal' && event.touches.length === 1) {
    ...
    const dragEnd = (event: PointerEvent) => {
        ...
        const distance = dragRegister.current.reduce((a, sample) => a + Math.abs(sample.dpos), 0);
