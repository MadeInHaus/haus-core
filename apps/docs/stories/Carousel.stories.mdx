...
Responsive props are passed to the carousel as CSS variables. Define these on the carousel's container element or on one of its ancestors.
...
Relative length values are computed relative to the width of the carousel's container.
