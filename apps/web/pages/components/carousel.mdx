...
- **`onPosition`** _(data: PositionData) => void (default: undefined)_<br />
  A callback that is called on every position update with visibility metadata.
  It receives a PositionData object:
  - **`containerSize`**: _number_ — The size of the carousel container element.
  - **`snapPosition`**: _number_ — Specifies where within the container items should snap into place.
  - **`gap`**: _number_ — The gap between items.
  - **`visibleItems`**: _VisibleItem[]_ — Array of currently visible items. Each item is an object
    containing `index` (number), `element` (HTMLElement), `startPos` (number), and `endPos` (number).
- **`onSnap`** _(index: number, element: HTMLElement) => void (default: undefined)_<br />
  A callback that is called when the carousel snaps to an item.
  It receives the snapped item's `index` and its `HTMLElement`.
...
- **`--carousel-autoadvance`** _number (default: 0)_<br />
  Set to `1` or `-1` to automatically advance the carousel step by step in the specified direction.
- **`--carousel-autoadvance-delay`** _time value (default: 5s)_<br />
  The delay between automatic advances. Supports time units (e.g. `5s` or `5000ms`).
- **`--carousel-autoadvance-duration`** _time value (default: 700ms)_<br />
  The duration of the step animation. Supports time units (e.g. `700ms` or `0.7s`).
...
- **`next(duration)`**<br />
  Programmatically moves the carousel to the next item. The `duration` in milliseconds is optional (default: 700).
- **`prev(duration)`**<br />
  Programmatically moves the carousel to the previous item. The `duration` in milliseconds is optional (default: 700).
- **`refresh()`**<br />
...
- The carousel can currently not be used as a controlled component. Even though it has a
  ...
  initialize the active item on mount). Instead, use `moveIntoView(index, options)` or
  `next(duration)` or `prev(duration)` to programmatically move the carousel items.
