...
- **`enableVerticalScroll`** _boolean (default: false)_<br/>
  ...
  scroll horizontally when swiping or scrolling up or down (or, when
  `direction` is set to `"vertical"`, scroll vertically).
...
- **`--carousel-item-size`** _length value (default: 0)_<br />
  ...
  length value !== 0 if _all items_ are the same width. This helps with performance because
  the carousel doesn't have to measure the size of each item.
