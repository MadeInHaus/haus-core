...
The TexturalVideo component renders a HTML5 video element with primary and secondary video sources. It uses the useIntersectionObserver hook to play/pause the video based on whether it is currently in the viewport. It also has options to set the `className` and `poster` props, and other attributes of the HTML `<video>` element.
