...
<Tabs items={['npm', 'yarn', 'pnpm']}>
    <Tab>
        ```bash copy
        npm install @madeinhaus/theme
        ```
    </Tab>
    <Tab>
        ```bash copy
        yarn add @madeinhaus/theme
        ```
    </Tab>
    <Tab>
        ```bash copy
        pnpm add @madeinhaus/theme
        ```
    </Tab>
</Tabs>
...
To enable themes in your app, you have to add the components ThemeProvider to
`_app.tsx` and ThemeScript to `_document.tsx`, as follows:
...
import { ThemeProvider } from '@madeinhaus/theme';
...
import { ThemeScript } from '@madeinhaus/theme';
...
import { useTheme } from '@madeinhaus/theme';
...
const Index = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => setTheme('auto')}>Set theme to 'auto'</button>
    </div>
  );
};
